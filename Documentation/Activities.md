...
The core owns physical-notch spacing and default content dimensions. Providers, including adapted activities, may publish deterministic full and minimal presentation widths when their content needs less or more space. The selected stack is used for both rendering and closed-notch sizing so the two paths do not diverge. Each provider supplies accessory, full, and minimal content. Live views must not resize the notch or change navigation. Keep live content inexpensive: update only while its displayed data changes and derive elapsed time from timestamps rather than accumulated ticks.
